...
if debug == True:
	print "Debugging and printing lots of stuff"
...
* Run the program on a single server


* Instead of diving lanes run lanes as a single job.

    - what do i do with umapped reads ? should i return unmapped reads and then run through another cycle of the pipeline
    using slower read aligners that split the reads into smaller groups ? 

    
2. property of file is length of file in lines ???
   determine max length/ number of lines

   find file details use info from file on each node to select the information needed.
...
run_all_files_dict[cp_file[-1]] = ["%s_%s_%s"%(read_file_name1,aligner,cp_file[-1]),read_file_name1,read_file_name2]

changed the form of the run_all dict to add in the orignial reads - this needs to be changed in launch and make sure it doesnt affect anywhere else
and finally also look at post analysis to make the required changes to make that work effectively. after this hopefully its smooth sailing!
'''
...
def collect_stats(job_list,cluster_data,run_data,tag):
	...
	while 1:
		...
		for job in job_list:
			...
			if job in jobs_done:
				...
				#print [cluster_data["cluster_acct"],cluster_data["cluster_qacct_us"],run_data["username"],cluster_data["cluster_js"],job]
				
				error = False
				for line in stats_proc.stderr:
					#print line
	...
	if log_file_dir != "":
		open(os.path.join(log_file_dir,"%s_stats_%s_run.txt"%(tag,aligner)),"w").writelines(stats_list)
	else:
		open(os.path.join(aligner_data[aligner]["output_directory"],"%s_stats_%s_run.txt"%(tag,aligner)),"w").writelines(stats_list)

def delete_tmp_dirs(run_dirs,run_data,diag,run_all):
...
def split_file(file_to_split,desired_file_size,dest,not_strict):
	...
	log_list.append(str(desired_file_size)+"\t"+str(split_number)+"\n")
	
	if split_number < 1000000000000000:
		file_list = []
		outlist = []    
		count = 1
		tot_c = 0
		...
		for line in open(file_to_split):
			...
			if tot_c >= split_number:
				...
				#print "SPLITTING ON LINE"
				#print line
				
				file_handle.writelines(outlist)
				file_handle.close()
...
def cat_files_sam(final_file_name,work_directory_master,file_count):
	...
	temp_file = open(final_file_name+"temp","w")
	final_file = open(final_file_name,"w")
	...
	for L, fou in dispatch.values():
	    if L: fou.writelines(L)
	
	temp_file.close()
	#eek(0)
	temp_file = open(final_file_name+"temp")
	shutil.copyfileobj(temp_file, final_file)
	os.remove(final_file_name+"temp")
...
def get_queue_list_LSF(username="warren"):

    cluster_proc = subprocess.Popen(['bjobs','-w','-u',username],stdout=subprocess.PIPE,stderr=subprocess.PIPE)
    
    q_list = []
    for line in cluster_proc.stdout:
        if line.lower().startswith("job") or line.startswith("--"):
            continue
...
def queue_info_LSF(total_files,tag,username="emmett"):
...
def queue_info_eqw(total_files,tag,username="emmett"):
	...
	#take only jobs for this current script - identified by tag
	for q in q_list:
	    if q[2].startswith(tag) and q[4].lower() != "eqw":
		    ...
		    cluster_del = subprocess.Popen(['qdel',q[0]],stdout=subprocess.PIPE,stderr=subprocess.PIPE)
		    
		    print >> sys.stderr,"Deleted job with EQW status."     
...
def queue_info_eqw_LSF(total_files,tag,username="warren"):
    
	q_list = get_queue_list_LSF(username)
	...
	#take only jobs for this current script - identified by tag
	for q in q_list:
		script_name = os.path.split(q[7])[1]
	...
	if debug == True:
		print "ORIGINAL Q LISt: ", q_list
		print "JOBS SELECTED: ",q_jobs
	...
	for q1 in q_jobs:
		...
		if debug == True:
			print "Q1 :",q1
			print "RESULT : ",os.path.split(q1[7])[1][:10]
			print "XX1",int(os.path.split(q1[7])[1].split("_")[1])
		    
		names_jobs_present.append(os.path.split(q1[7])[1][:10])
		
		number_jobs.append(int(os.path.split(q1[7])[1].split("_")[1]))
...
'''
def queue_info_eqw_LSF(total_files,job_list,username="emmett"):
	...
	#take only jobs for this current script - identified by tag
	for q in q_list:
	    if q[0] in job_list #and q[].lower() != "eqw":
		    ...
		    cluster_del = subprocess.Popen(['bkill',q[0]],stdout=subprocess.PIPE,stderr=subprocess.PIPE)
...
def build_cluster_Script(node_aligner_part,node_unmapped_part,aligner_part,unmapped_part,read_file_name1,read_file_name2,script_name,dir_job_done,files_created,local_copy,tmp_node_dir,mem_for_job,run_master,run_all,paired_dict,diag=0):
	
	original_name1 = read_file_name1
	original_name2 = read_file_name2
	### Build bash script for each node!!!        
	output_files = []
	
	#### if running batch mode a collection of files
	run_all_files_dict = {} 
	...
	unique_id = os.path.split(read_file_name1)[1][:-3]
	# only engage local copy script if it is not in testmode
	if diag != 1:
		if local_copy == True:
			
			local_read1= os.path.join(tmp_node_dir,"local_read1"+unique_id)
			
			files_created.append(local_read1)
			
			if paired == True:
				local_read2= os.path.join(tmp_node_dir,"local_read2"+unique_id)
		...
		for read_num in read_iter:
			...
			for line in run_data["read_pp"].strip().split("|"):
				...
				if readpp_step_number == read_step_number:
					final_read_name = "%sfinal_Read%s_%s"%(pp_read_prefix,str(read_num),read_file_name1)
	...
	if diag != 1:
		...
		if run_all == True and local_copy != True:
			
			
			for cp_file in output_files:
	
				script_cmd+="cp %s %s\n"%(cp_file,"%s_%s_%s"%(read_file_name1,aligner,cp_file[-1]))  
				run_all_files_dict[cp_file[-1]] = ["%s_%s_%s"%(read_file_name1,aligner,cp_file[-1]),read_file_name1,read_file_name2]
		
		elif run_all == True and local_copy == True:
			
			if debug == True:
				print "RUNNING RUN ALL AND LOCAL COPY !!!"
			...
			for cp_file in output_files:
				...
				script_cmd+="cp %s %s\n"%(cp_file,"%s_%s_%s"%(original_name1,aligner,cp_file[-1]))  
				run_all_files_dict[cp_file[-1]] = ["%s_%s_%s"%(original_name1,aligner,cp_file[-1]),original_name1,original_name2]
	...
	return script_cmd,run_all_files_dict

# collect aligner information before making scripts
def aligner_info():
        print >> sys.stderr,"\n\nAligner Information\n======================="
        print >> sys.stderr,"There are built-in scripts for 5 read aligners, in order to activate these scripts please select an aligner and specify the FULL PATH to the aligner executable.\nPLEASE NOTE: THIS PATH MUST BE VISIBLE TO ALL CLUSTER NODES.\nAvailable aligners are: 'bowtie','bwa','soap','blat'. If the aligner is not available on the system please type 'na'\n"
        aligner_dict = {}
        
        aligner_name = ""
        aligner_location = ""
        
        for align_ in ["bowtie","bwa","blat","soap"]:
                
                while 1:        
                        clusteralign_root=raw_input("ALIGNER: %s\nPlease specify FULL path to this aligner (ie. /mnt/lustre/bowtie-0.0.10/bowtie). PATH MUST be visible to all cluster nodes or point to the local installation on the node. If not installed type 'na':\n"%(align_))
                        
                        if clusteralign_root.lower().strip() == "na":
                            print >> sys.stderr,"Aligner not present, skipping..."
                            break
                        else:
                            if os.path.isfile(clusteralign_root):
                                print >> sys.stderr,"Aligner path confirmed"
                                aligner_dict[align_] = clusteralign_root
                                break
                            else:
                                print >> sys.stderr,"File does not exist, please try again."
...
def create_scripts():
        ...
        for config_file in config_files:
                ...
                if config_file.split(".")[0].lower() in aligner_dict.keys():
                        ...
                        for line in open(os.path.join(scripts_dir,config_file)):
                                ...
                                line = line.replace("SOFTWAREDIR",aligner_dict[config_file.split(".")[0].lower()])
		...
		if config_file.split(".")[0].lower() == "cluster":
			write_list = [] 
			for line in open(os.path.join(scripts_dir,config_file)):
...
def read_config_file(config_paths,aligner,paired,input_out,log_list,input_read1,input_read2):
	...
	for config_path in config_paths:
		for line in open(config_path):
			...
			elif line.strip().startswith("cluster_del_command"):
				cluster_data[line.strip().split("=")[0].strip()] = line.strip().split("=")[1].strip()
				
			elif line.strip().startswith("cluster_qstat"):
				cluster_data[line.strip().split("=")[0].strip()] = line.strip().split("=")[1].strip()
			
			elif line.strip().startswith("cluster_qstat_user_param"):
				cluster_data[line.strip().split("=")[0].strip()] = line.strip().split("=")[1].strip()
...
def create_all_directories(run_data,aligner_data):
	...
	if log_file_dir != "":
		create_dir(log_file_dir,overwrite=0)
	...
	##### for running on the local server if specified
	if run_master == True:
		
		run_dirs["analysis_dir_master"] = os.path.join(run_data["dir_for_read_parts"],tag,"analysis_master_")
...
def launch_script(paired,aligner_data,run_data,run_dirs,tag,local_copy,not_strict,log_list,core_limit,paired_dict):
	...
	if paired_dict == "":
		...
		# Run a normal analysis using a single file and splitting it into smaller pieces
		...
		print >> sys.stderr,"[%s] Read files split into fragments of size %s. Total of %s files."%(time_now(),run_data["length_parameter"],len(file_list))
		log_list.append("[%s] Read files split into fragments of size %s. Total of %s files."%(time_now(),run_data["length_parameter"],len(file_list)))

	else:
		# run each file as a seperate job output = list of files to be evalutated [[s1_1,s1_2],[s2_1,s2_2]]
		file_list = []
		for root in paired_dict:
			for prefix in paired_dict[root]:
				pref_list = []
				for fn_p in paired_dict[root][prefix]:
					pref_list.append(fn_p)
					
				file_list.append(pref_list)
		if debug == True:
			print file_list
			raw_input("File list above...")
	...
	# script number for the single node mode - to avoid having everything in a single directory		
	script_number = 1	
	...
	for file_ in file_list:
		...
		########## CLUSTER SUBMISSION CONVERSION TO SINGLE SERVER
		
		if run_master == True:
			run_data["tmp_node_dir"] = run_dirs["analysis_dir_master"]+str(script_number)
			
			
		if run_all == True:
			# try to rename as many variables as i can to avoid having to do this later
			run_dirs["tmp_results_dir"] = os.path.split(file_[0])[0]
			
			print "tmp_results_dir",os.path.split(file_[0])[0]
	    
		#files included
		files_created = []
		
		# create file paths for unmapped files  
		# unmapped variable not set
		if paired == True:
			print "\nPAIRED"
			print "original",file_
			...
			script_cmd,run_all_files=build_cluster_Script(node_aligner_part,node_unmapped_part,aligner_part,unmapped_part,file_[0],file_[1],script_name,run_dirs["dir_job_done"],files_created,local_copy,run_data["tmp_node_dir"],run_data["node_mem"],run_master,run_all,paired_dict)
			script_number +=1
			
			'''
			print file_
			print file_[0]
			print os.path.split(file_[0])[0]
			raw_input()
			'''
			
			if os.path.split(file_[0])[0] in run_all_files_master_dict:
				run_all_files_master_dict[os.path.split(file_[0])[0]].append(run_all_files)
			else:
				run_all_files_master_dict[os.path.split(file_[0])[0]] = [run_all_files]
		else:
			if debug == True:
				print "\nSINGLE"
				print "original",file_
			...
			script_name = "%s_%s_%s.sh"%(tag,count_unmap,aligner)
			script_cmd,run_all_files=build_cluster_Script(node_aligner_part,node_unmapped_part,aligner_part,unmapped_part,file_,read_file_name2,script_name,run_dirs["dir_job_done"],files_created,local_copy,run_data["tmp_node_dir"],run_data["node_mem"],run_master,run_all,paired_dict)
			script_number +=1
			
			print file_
			
			if os.path.split(file_)[0] in run_all_files_master_dict:
				run_all_files_master_dict[os.path.split(file_)[0]].append(run_all_files)
				
			else:
				run_all_files_master_dict[os.path.split(file_)[0]] = [run_all_files]
		#print script_cmd
		#raw_input()
	...
	#core_limit = 4
	count = 0
	for bowtie_bash_script in bash_scripts:
		
		#print count, core_limit
		if count == core_limit:
			#print count
			while 1:
				total_jobs,numbers_not_present,names_jobs_present = queue_info(core_limit,tag)
				
				#print names_jobs_present
				if len(names_jobs_present) < core_limit:
					print "Job finished, submitting next one..."
					count = count-len(names_jobs_present)
					break
		...
		if run_master == False:		
			...
			if log_file_dir != "":
				cluster_error_file = os.path.join(log_file_dir,"%s.err"%(os.path.split(bowtie_bash_script)[-1])) 
				cluster_output_file =os.path.join(log_file_dir,"%s.out"%(os.path.split(bowtie_bash_script)[-1])) 
			else:
				cluster_error_file = os.path.join(run_dirs["cluster_err_dir"],"%s.err"%(bowtie_bash_script)) 
			...
			if cluster_type.upper() == "SGE" or cluster_type.upper() != "LSF":
				...
				if debug == True:
					...
					print " ".join(clus_stat)
				
				cluster_proc = subprocess.Popen(clus_stat,stdout=subprocess.PIPE,stderr=subprocess.PIPE)
				count += 1
				
				cluster_lines.append(clus_stat)
			
				
				
			#print bowtie_bash_script
			...
			for line in cluster_proc.stdout:
				...
				job_name = ""
				job_id_p = ""
				if cluster_type.upper() != "LSF":
					job_id_p = line.split('"')[1]
					job_list.append(line.split()[2])
				else:
					job_id_p = os.path.split(bowtie_bash_script)[1][:10]
					job_list.append(line.split('<')[1].split(">")[0])
					
				
				
					
				job_name_list.append(job_id_p)
				
				time_dict[job_id_p] = [ztime.time()]
				
			print "job_list=",job_list
			print "job_name_list=",job_name_list
			
			########################### CLUSTER SUBMISSION ^^^^^^^^^^^^^^^^^^^^^6
	...
	log_list.append("[%s] All Jobs submitted successfully, %s Jobs at a time"%(time_now(),core_limit))
	return total_files,job_name_list,job_list,bash_scripts,cluster_lines,log_list,time_dict,run_all_files_master_dict
...
def check_file_sam(work_directory_master,file_count):
	...
	if sam == True and empty_file == False:
		#print "not sam"
		return True
	#print "sam"
	return False

def manage_jobs(cluster_data,run_data,run_dirs,total_files,tag,job_name_list,bash_scripts,cluster_lines,log_list,time_dict,job_list):
	# check to see when all jobs have completed on the cluster and concatenate final files
	
	# to help debug the job resetting system
	job_dead = 0
	
	### make a dictionary for jobs
	jobname_dict = {}
	for jobname_ in job_name_list:
		
		jobname_dict[jobname_] = job_list[job_name_list.index(jobname_)]
		
	if debug == True:
		print "JOB DICT BELOW"
		pp.pprint(jobname_dict)
	...
	####### time list to estimate run time based on how long jobs take to complete - this is used to kill jobs that exceed a default of 20% more time than the majority of jobs
	time_list = []
	avg_runtime = 0.0
	extra_time = 30
	...
	while 1:
		...
		if debug == True:
			...
			print "final_prog_list",final_prog_list
		...
		if debug == True:
			print "numbers_not_present",numbers_not_present
			print "names_jobs_present=",names_jobs_present
			print "failed_jobs=",failed_jobs
			print "completed_jobs=",completed_jobs
			print "job_name_list=",job_name_list
		...
		for job_ in job_name_list:
			...
			if job_[:10] not in names_jobs_present:
				...
				if job_[:10] in final_prog_list:
					...
					time_dict[job_].append(ztime.time())
					
					time_list.append(time_dict[job_][1] - time_dict[job_][0])
		...
		if len(failed_jobs) > 0:
			...
			for fjob in failed_jobs:
				...
				if fjob in jobs_resubmitted:
					if debug == True:
						
						print "Job %s has already been submitted and will not be resubmitted."%(fjob)
					continue
				...
				if fjob not in repeat_jobs:
					...
					print >> sys.stderr,"This job has not completed successfully, resubmitting: %s"%(fjob)
					log_list.append("This job has not completed successfully, resubmitting: %s"%(fjob))
					#failed_jobs.append(fjob)
					
					time_dict[fjob][0] = ztime.time()
					...
					for line in cluster_proc.stdout:
						#job_list.append(line.split()[2])
						########## so adding a line to pull out job id and over-write it in file
						
						if line.startswith("Your job"):
							
							new_id = line.split("Your job")[1].split("(")[0].strip()
					...
					if debug == True:
						print fjob
						print new_id
						print "adding fjob to newid"
						
					jobname_dict[fjob] = new_id
		...
		elif total_files - len(final_prog_list) < total_files/5 or total_files - len(final_prog_list) < 3 :
			# if only 20% of files remain to be completed check to see how long they are taking and then decide to wait some more or KILL EM!
			
			if len(final_prog_list) - total_files < 3:
				
				if len(time_list) <= 1:
					continue
				
				print "final prog list: ",final_prog_list
				...
				#avg_runtime = float(sum(time_list))/len(time_list)
				threshold_value = 0
				if (float(sum(time_list))/len(time_list)) > 3000.0:
					
					print "Threshold value is large, using a more stringent cut off..."
					threshold_value = (float(sum(time_list))/len(time_list))* 1.1
				else:
					threshold_value =float(max(time_list))* 1.5
				
				#print "avg_run time: ",avg_runtime
				print time_list
				print "threshold: ", threshold_value
				
				for job_1 in job_name_list:
					
					if job_1 not in completed_jobs:
						
						run_time = time_atm - time_dict[job_1][0]
						
						if murder_job == True and job_dead == 0:
							
							if total_files - len(completed_jobs) == 1:
								
								job_dead = 1
								
								print "Killing the last job, mwahahahahaha!"
								
								run_time = 1000000000000000000
						...
						if run_time > threshold_value:
							...
							time_dict[job_1][0] = ztime.time()
							
							print jobname_dict[job_1]
							
							for line in cluster_proc1.stdout:
								
								print "out",line
							
							for line in cluster_proc1.stderr:
...
'''
def manage_jobs_local(cluster_data,run_data,run_dirs,total_files,tag,job_name_list,bash_scripts,cluster_lines,log_list,time_dict,job_list,run_master):
	# check to see when all jobs have completed on the cluster and concatenate final files
	...
	while 1:
		...
		if len(final_prog_list) == total_files:
			...
			if len(final_prog_list) - total_files < 3:
				...
				print "threshold: ",float(max(time_list))* 1.5 
				...
				for job_1 in job_name_list:
					...
					if job_1 not in completed_jobs:
						...
						if run_time > float(max(time_list))*1.5:
							
							print >> sys.stderr,"%s has exceeded the average run time by more than %s percent. Deleting and rerunning."%(job_1,extra_time)
							
							cluster_proc1 = subprocess.Popen(["qdel",job_list[job_name_list.index(job_1)]])
...
def testmode(run_dirs,aligner_data):
		...
		script_cmd,run_all_files=build_cluster_Script(os.path.join(run_dirs["diag_dir"],"node_aligner_part"),os.path.join(run_dirs["diag_dir"],"node_aligner_part"),os.path.join(run_dirs["diag_dir"],"aligner_master_part"),os.path.join(run_dirs["diag_dir"],"aligner_master_part"),aligner_data[aligner]["test_read_file1"],aligner_data[aligner]["test_read_file2"],script_name,done_files_diag,[],local_copy,os.path.join(run_dirs["diag_dir"],"node_dir"),"100",run_master,False,paired_dict="",diag=1)
...
def arrange_read_files(aligner_data,paired):
	# arrange files that paired reads are collected together  
	file_list = []
	paired_dict = {}
...
def arrange_read_files_all_dirs(aligner_data,paired,select_dirs_with):
	# arrange files that paired reads are collected together  
	file_list = []
	paired_dict = {}
	...
	read_suffix = aligner_data[aligner]["read_suffix"].strip()
	
	print read_suffix
	
	print aligner_data[aligner]["read_directory"]
	
	for root,dirs,files in os.walk(aligner_data[aligner]["read_directory"]):
		...
		if len(files) > 0:
			...
			for file_ in files:
				...
				prefix_index = file_.find(aligner_data[aligner]["read_prefix"])
				
				#print "pref_ind",prefix_index
				
				if prefix_index > -1 and file_.endswith(read_suffix):
					
					if len(select_dirs_with) > 0 :
						
						for pattern in select_dirs_with:
							
							if pattern not in root:
								print "Skipping this directory,",pattern
								print root
					...
					#if paired == True:
					pair_prefix = file_[prefix_index:prefix_index+aligner_data[aligner]["paired_prefix_length"]]
					...
					print aligner_data[aligner]["paired_prefix_length"]
					
					#raw_input("check")
					
					if root not in paired_dict:
						paired_dict[root] = {}
										
					
					if pair_prefix not in paired_dict[root]:
						paired_dict[root][pair_prefix] = [os.path.join(root,file_)]
					else:
						paired_dict[root][pair_prefix].append(os.path.join(root,file_))
...
def build_post_analysis_script(run_data,aligner_data,final_output_files,diag,paired):
	...
	'''
	aligner_data[aligner]["read_file1"]
	aligner_data[aligner]["read_file2"]
	'''
	if diag != True:
		...
		post_analysis_name = os.path.join(aligner_data[aligner]["output_directory"],"%s_%s"%(os.path.split(read_file_name1)[1],"post_processing"))
		
		print "post analysis name: ",post_analysis_name
	...
	for line in run_data["post_analysis"].strip().split("|"):
		...
		if line.find("Results_1") > -1:	
			...
			line =line.replace("Results_1",final_output_files["1"][0])
			
		if line.find("Results_2") > -1:
			#prev_output2 = final_output_files["2"]	
			line =line.replace("Results_2",final_output_files["2"][0])
			
		if line.find("Results_3") > -1:
			#prev_output3 = final_output_files["3"]
			line =line.replace("Results_3",final_output_files["3"][0])
...
def launch_post_analysis_commands(run_data,run_dirs,aligner_data,final_output_files,paired):
	...
	if debug == True:
		print "post script",post_script_cmd
...
def pipeline_instance(aligner_data,run_data,local_copy,paired,diag,log_list,run_all,paired_dict=""):
	...
	total_files,job_name_list,job_list,bash_scripts,cluster_lines,log_list,time_dict,run_all_files = launch_script(paired,aligner_data,run_data,run_dirs,tag,local_copy,not_strict,log_list,core_limit,paired_dict)

	# manage and resubmit failed jobs
	add_to_stats_list,final_output_files,log_list = manage_jobs(cluster_data,run_data,run_dirs,total_files,tag,job_name_list,bash_scripts,cluster_lines,log_list,time_dict,job_list)
	...
	dur_secs = end-start
	print >> sys.stderr, "Pipeline exectution complete [%s elapsed]\n"%(duration)
	print >> sys.stderr, "Seconds: [%s elapsed]"%(dur_secs)
	
	log_list.append("Seconds: [%s elapsed]\n"%(dur_secs))
	log_list.append("Pipeline exectution complete [%s elapsed]\n"%(duration))
	
	if log_file_dir != "":
		open(os.path.join(log_file_dir,tag+"_logfile_"+aligner+"_cluster.txt"),"w").writelines(log_list)  
	...
	if run_all == False:
		# standard file output is used if the file is split
		launch_post_analysis_commands(run_data,run_dirs,aligner_data,final_output_files,paired)
	elif run_all == True:
		# if a batch job of whole lanes is run then the final files are slightly different
		# run_all_files contains all the files created
		
		if debug == True:
			pp.pprint(run_all_files)
			
			raw_input()
		
		
		for read_set_path in run_all_files:
			
			for dict_key in run_all_files[read_set_path]:
				
				if debug == True:
					print dict_key
					####### post sscript --- need to have correct read files - change dict for each iter. including 
				
					print "PATH CHANGED TO::",read_set_path
				aligner_data[aligner]["output_directory"] = read_set_path
				aligner_data[aligner]["read_file1"] = dict_key["1"][1]
				aligner_data[aligner]["read_file2"] = dict_key["1"][2]
				
				launch_post_analysis_commands(run_data,run_dirs,aligner_data,dict_key,paired)
			
		
	if debug == True:	
		# delete all temporary dirs
		print run_dirs["dir_for_read_parts"]
		raw_input("One last chance to check dirs")
	delete_tmp_dirs(run_dirs,run_data,diag,run_all)
...
run_all = False  # run all files specified simultaneously without splitting...
select_dirs_with = [] # select specific directories to align
...
for arg_ in sys.argv[1:]:
	...
	elif arg_ == "--run_master":
		run_master = True
		
	elif arg_.lower().startswith("--select_dirs="):
		
		select_dirs_with = arg_.split("=")[1].split(",")
		print "select_dirs_with",select_dirs_with
		
		
	elif arg_.lower().startswith("--log_file_dir="):
		
		log_file_dir = arg_.split("=")[1]

	elif arg_.lower().startswith("--core-lim="):
		try:
			core_limit = int(arg_.split("=")[1])
		except:
			sys.exit("Invalid argument for the '--core-lim' parameter. Value must be an integer above zero.")
...
if aligner_data[aligner]["read_directory"].lower().strip() != "none":
	...
	file_list,paired_dict =arrange_read_files_all_dirs(aligner_data,paired,select_dirs_with)	
	...
	if run_all == True:
		...
		''' ######## This is intended to allow users to select a directory.... not enabled at present 
		# create all dirs for output
		for root in paired_dict:
			
			for lane in paired_dict[root]:
			
				aligner_data[aligner]["output_directory"] = os.path.join(root,lane)
		'''
		
		final_output_files = pipeline_instance(aligner_data,run_data,local_copy,paired,diag,log_list,run_all,paired_dict)
		...
		if paired == True:
			...
			for lane in paired_dict:
				...
				if cnt > 0:
					diag=0
				final_output_files = pipeline_instance(aligner_data,run_data,local_copy,paired,diag,log_list,run_all)
				cnt+=1
		else:
			cnt=0
