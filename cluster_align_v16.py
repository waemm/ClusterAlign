...
def cat_files(final_file_name,work_directory_master,file_count):
	...
	destination = open(final_file_name,'wb')
	...
	if len(file_count) > 1:
		...
		shutil.copyfileobj(open(bowtie_file,'rb',),destination,length=6273741824)
...
def cat_files_sam(final_file_name,work_directory_master,file_count):
	...
	# only for @ headers    
        for bowtie_file in file_count:
            ...
            tmp_count = 0
	    file_handle = open(os.path.join(work_directory_master,bowtie_file))
			...
			file_seek[bowtie_file] = file_handle.tell()
			final_file.writelines(tmp_list)
			file_handle.close()
			break
	...
	for bowtie_file in file_count:
	    ...
	    file_handle2 = open(os.path.join(work_directory_master,bowtie_file))
	    
	    file_handle2.seek(file_seek[bowtie_file])
...
def cat_files_sam(final_file_name,work_directory_master,file_count):
	...
	# only for @ headers    
	    ...
	    file_handle = open(os.path.join(work_directory_master,bowtie_file))
            for line in file_handle:
                if line.startswith("@"):
			#print line
			header_data += len(line) # +1 for the end of line thingy
			#file_seek[bowtie_file] = file_handle.tell()
			...
			final_file.writelines(tmp_list)
			file_seek[bowtie_file] = header_data
			file_handle.close()
			break
	...
	for bowtie_file in file_count:
	    ...
	    #print "here"
	    #print file_seek[bowtie_file]
	    if bowtie_file in file_seek:
		    file_handle2.seek(file_seek[bowtie_file])
	    else:
		    continue
            for line in file_handle2:
...
def build_cluster_Script(node_aligner_part,node_unmapped_part,aligner_part,unmapped_part,read_file_name1,read_file_name2,script_name,dir_job_done,files_created,local_copy,tmp_node_dir,mem_for_job,diag=0):
...
def launch_script(paired,aligner_data,run_data,run_dirs,tag,local_copy,not_strict,log_list,core_limit):
	...
	for file_ in file_list:
		...
		if paired == True:
			...
			script_cmd=build_cluster_Script(node_aligner_part,node_unmapped_part,aligner_part,unmapped_part,file_[0],file_[1],script_name,run_dirs["dir_job_done"],files_created,local_copy,run_data["tmp_node_dir"],run_data["node_mem"])
			...
			script_name = "%s_%s_%s.sh"%(tag,count_unmap,aligner)
			script_cmd=build_cluster_Script(node_aligner_part,node_unmapped_part,aligner_part,unmapped_part,file_,read_file_name2,script_name,run_dirs["dir_job_done"],files_created,local_copy,run_data["tmp_node_dir"],run_data["node_mem"])
...
def testmode(run_dirs,aligner_data):
		...
		script_cmd=build_cluster_Script(os.path.join(run_dirs["diag_dir"],"node_aligner_part"),os.path.join(run_dirs["diag_dir"],"node_aligner_part"),os.path.join(run_dirs["diag_dir"],"aligner_master_part"),os.path.join(run_dirs["diag_dir"],"aligner_master_part"),aligner_data[aligner]["test_read_file1"],aligner_data[aligner]["test_read_file2"],script_name,done_files_diag,[],local_copy,os.path.join(run_dirs["diag_dir"],"node_dir"),"100",diag=1)
...
def pipeline_instance(aligner_data,run_data,local_copy,paired,diag,log_list):
	...
	total_files,job_name_list,job_list,bash_scripts,cluster_lines,log_list = launch_script(paired,aligner_data,run_data,run_dirs,tag,local_copy,not_strict,log_list,core_limit)
